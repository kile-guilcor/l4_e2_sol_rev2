...
MEMORY
    {
    FLASH (rx) : ORIGIN = (0xe000000 + 0x30000), LENGTH = (0x11d000 - 0x0)
    RAM (wx) : ORIGIN = 0x2f000000, LENGTH = (760 * 1K)
    DEVNULL_ROM (rx) : ORIGIN = ((0xe000000 + 0x30000) + (0x11d000 - 0x0)), LENGTH = 262144
    DMA_RAM21 ( rw ) : ORIGIN = (797556736), LENGTH = (16384) DMA_RAM3x_APP ( rw ) : ORIGIN = (801185792), LENGTH = (4096) PMLocalRamfunc ( rw ) : ORIGIN = (570457856), LENGTH = (192) pm_s2ram_stack ( rw ) : ORIGIN = (570458056), LENGTH = (16) mcuboot_s2ram_context ( rw ) : ORIGIN = (570458072), LENGTH = (8) pm_s2ram_context ( rw ) : ORIGIN = (570458080), LENGTH = (32)
 ...
 *(.iplt)
 ...
 __rom_region_start = (0xe000000 + 0x30000);
    rom_start :
 {
...
ztest :
{
 ...
 net_socket_service_desc_area : { _net_socket_service_desc_list_start = .; KEEP(*(SORT_BY_NAME(._net_socket_service_desc.static.*))); _net_socket_service_desc_list_end = .;; } > FLASH
...
uicr_periphconf_entry (COPY) : SUBALIGN(4)
{
 _uicr_periphconf_entry_list_start = .; KEEP(*(SORT_BY_NAME(._uicr_periphconf_entry.static.*))); _uicr_periphconf_entry_list_end = .;;
} > DEVNULL_ROM
/DISCARD/ :
{
 KEEP(*(.irq_info*))
...
.align16 :
{
 ...
 KEEP(*(.gnu.attributes))
    ...
    DMA_RAM21 (NOLOAD) : { __DMA_RAM21_start = .; KEEP(*(DMA_RAM21)) KEEP(*(DMA_RAM21.*)) __DMA_RAM21_end = .; } > DMA_RAM21 __DMA_RAM21_size = __DMA_RAM21_end - __DMA_RAM21_start; __DMA_RAM21_load_start = LOADADDR(DMA_RAM21); DMA_RAM3x_APP (NOLOAD) : { __DMA_RAM3x_APP_start = .; KEEP(*(DMA_RAM3x_APP)) KEEP(*(DMA_RAM3x_APP.*)) __DMA_RAM3x_APP_end = .; } > DMA_RAM3x_APP __DMA_RAM3x_APP_size = __DMA_RAM3x_APP_end - __DMA_RAM3x_APP_start; __DMA_RAM3x_APP_load_start = LOADADDR(DMA_RAM3x_APP); PMLocalRamfunc (NOLOAD) : { __PMLocalRamfunc_start = .; KEEP(*(PMLocalRamfunc)) KEEP(*(PMLocalRamfunc.*)) __PMLocalRamfunc_end = .; } > PMLocalRamfunc __PMLocalRamfunc_size = __PMLocalRamfunc_end - __PMLocalRamfunc_start; __PMLocalRamfunc_load_start = LOADADDR(PMLocalRamfunc); pm_s2ram_stack (NOLOAD) : { __pm_s2ram_stack_start = .; KEEP(*(pm_s2ram_stack)) KEEP(*(pm_s2ram_stack.*)) __pm_s2ram_stack_end = .; } > pm_s2ram_stack __pm_s2ram_stack_size = __pm_s2ram_stack_end - __pm_s2ram_stack_start; __pm_s2ram_stack_load_start = LOADADDR(pm_s2ram_stack); mcuboot_s2ram_context (NOLOAD) : { __mcuboot_s2ram_context_start = .; KEEP(*(mcuboot_s2ram_context)) KEEP(*(mcuboot_s2ram_context.*)) __mcuboot_s2ram_context_end = .; } > mcuboot_s2ram_context __mcuboot_s2ram_context_size = __mcuboot_s2ram_context_end - __mcuboot_s2ram_context_start; __mcuboot_s2ram_context_load_start = LOADADDR(mcuboot_s2ram_context); pm_s2ram_context (NOLOAD) : { __pm_s2ram_context_start = .; KEEP(*(pm_s2ram_context)) KEEP(*(pm_s2ram_context.*)) __pm_s2ram_context_end = .; } > pm_s2ram_context __pm_s2ram_context_size = __pm_s2ram_context_end - __pm_s2ram_context_start; __pm_s2ram_context_load_start = LOADADDR(pm_s2ram_context);
.last_section :
{
  KEEP(*(.last_section))
} > FLASH
...
noinit (NOLOAD) :
{
    ...
    __kernel_ram_end = 0x2f000000 + (760 * 1K);
